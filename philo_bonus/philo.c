...
/*   Updated: 2022/10/22 16:00:11 by kpanikka         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char *argv[])
{
	...
	my_forks = sem_open("/mysemaphore",O_CREAT, S_IRUSR | S_IWUSR, ft_atoi(argv[1]));
	sem_unlink("/mydeath");
	my_death = sem_open("/mydeath", O_CREAT, S_IRUSR | S_IWUSR, 0);
	sem_unlink("/print");
	...
	while (++i < ft_atoi(argv[1]))
	{
		...
		{
			//philo[i].id == i
			philo[i].my_fork = my_forks;
			philo[i].print = print;
			philo[i].my_death = my_death;
			routine(&philo[i]);
			sem_post(my_death);
			exit(0);
		}
